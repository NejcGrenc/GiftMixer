...
public class ChainService {
    ...
    @Autowired
    private ChainRuleRepository chainRuleRepository;
    ...
    public List<ChainRule> getAllRules(Admin admin) {
        System.out.println("Fetching list of all rules under care of admin " + admin.getUsername());
        return chainRuleRepository.findByAdminId(admin.getId());
    }

    public ChainRule addBlockingRule(long adminId, long giverId, long receiverId) {
        System.out.println("Creating new rule");
        ChainRule chainRule = new ChainRule();
        chainRule.setAdminId(adminId);
        chainRule.setRuleType("BLOCKING");
        chainRule.setGiverId(giverId);
        chainRule.setReceiverId(receiverId);
        return chainRuleRepository.saveAndFlush(chainRule);
    }

    public void removeRule(long ruleId) {
        System.out.println("Deleting a rule");
        chainRuleRepository.deleteById(ruleId);
    }
}
